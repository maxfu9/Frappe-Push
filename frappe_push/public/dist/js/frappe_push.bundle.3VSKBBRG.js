(() => {
  // ../frappe_push/frappe_push/public/js/frappe_push.bundle.js
  ...
  frappe_push.init = function() {
    if (window.frappe_push_initialized)
      return;
  ...
  frappe_push.setup_firebase = function(config) {
    frappe.require([
      ...
      try {
        ...
        navigator.serviceWorker.register("/api/method/frappe_push.frappe_push.api.get_service_worker", { scope: "/" }).then((registration) => {
          console.log("Frappe Push: Service Worker registered with scope:", registration.scope);
          return navigator.serviceWorker.ready;
        }).then((registration) => {
          return new Promise((resolve) => setTimeout(() => resolve(registration), 1e3));
        }).then((registration) => {
          ...
          function show_foreground_notification(payload) {
            ...
            const offset = existing_notifications.length * 100;
            const notify_html = `
          ...
          function show_subscription_banner() {
            if (document.getElementById("frappe-push-banner"))
              return;
            const is_guest = frappe.session.user === "Guest";
            const title = is_guest ? __("Stay Updated") : __("Direct Alerts");
            const message = is_guest ? __("Get real-time updates on your orders and exclusive offers from Europlast.") : __("Receive instant alerts for assignments, mentions, and system notifications.");
            const banner_html = `
							...
							">
								<div style="display: flex; justify-content: space-between; align-items: flex-start;">
									<div style="font-weight: 700; font-size: 18px; color: #1a1a1a;">${title} \u{1F514}</div>
...
//# sourceMappingURL=frappe_push.bundle.3VSKBBRG.js.map
